...
function stir_or_set() {
    if (p.init_position) {
        for(let y=0;y<=p.grid[1];y++) {
            for (let x = 0; x <= p.grid[0]; x++) {
                m[y][x]=p.init_position[y*p.grid[1] + x];
            }
        }
        for(let y=0;y<=p.grid[1];y++){
            for(let x=0;x<=p.grid[0];x++){
   ...
   // p.init_position = [];
    for(let y=0;y<=p.grid[1];y++){
        for(let x=0;x<=p.grid[0];x++){
            console.log('val:' + m[y][x]);
            //p.init_position[y*p.grid[0]+x] = m[y][x];
        }
    }
}
function stir_slots(){
...
function move_slot(s){
    ...
    if (newMoves.length) {
        //alert(newMoves.length);
        newMoves = newMoves.reverse();
        ms.push(...newMoves);
        try {
            if (p.moveListener){
                p.moveListener(newMoves);
            }
            const el = document.querySelector('#move_count');
            if (el) {
...
function check_slots(h){
    ...
    }if(check==o){handler()}//{setTimeout(handler/*()=>{alert('win') }*/,((p.time)?p.time*1000:0));} // <-- alert('win') script that runs at the end of the game
}
